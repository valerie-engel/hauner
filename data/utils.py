...
def load_graph(path, device='cpu', undirected=False, embedding=None, drop_labels=None):
    ...
    if drop_labels:
        graph = drop_nodes_by_label(graph, drop_labels, path)
...
def drop_nodes_by_label(graph, drop_labels, path):
    ...
    subgraph = _filter_graph_by_nodes(graph, keep_nodes_mask)
    # if not cfg.debug:
...
def _filter_graph_by_nodes(graph, keep_nodes):
    # subgraph edges
...
KG = load_graph(path=cfg.KG_path, drop_labels=['Biological_sample', 'Subject'])